...
Como muestra la figura \ref{fig:algo_ubicacion_different_size}, con particiones de distinto tamaño existen dos estrategias:
\begin{itemize}
...
\begin{figure}[!ht]
  ...
  \begin{subfigure}[c]{0.45\textwidth}
    ...
    \caption{1 cola de procesos por partición.}
  ...
  \label{fig:algo_ubicacion_different_size}
\end{figure}
...
Si todos los procesos residentes están bloqueados y no existe memoria suficiente para cargar otro proceso (incluso después de compactar), el sistema operativo debe seleccionar un proceso para intercambiar y liberar espacio.
...
Para resolverlo se distinguen tres tipos de direcciones:
\begin{itemize}
...
Los programas se cargan utilizando reubicación dinámica en tiempo de ejecución con soporte de hardware, manteniendo las referencias como direcciones relativas. Durante la ejecución, el hardware traduce automáticamente cada dirección relativa a una dirección física.
...
\begin{itemize}
  ...
  \item \textit{Registro límite:} contiene la dirección final de datos.
\end{itemize}
\begin{figure}[!ht]
  \centering
...
Luego se verifica que \(\text{dir}_{\text{física}} \leq \text{límite}\). Si está dentro de los límites, la ejecución continúa; si no, se genera una interrupción al SO.
