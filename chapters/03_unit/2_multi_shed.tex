\section{Planificación en Multiprocesadores, Multinúcleo y RTOS}
...
\subsection{Planificación en Multiprocesadores y Multinúcleo}
...
\begin{figure}[!ht]
  \centering
  % Tabla o esquema de los niveles de granularidad
  \caption{Clasificación del paralelismo según la frecuencia de sincronización: independiente, muy gruesa, gruesa, media y fina. A medida que aumenta la frecuencia de sincronización, también aumenta la complejidad de la planificación.}
\end{figure}
...
\begin{figure}[!ht]
    \centering
    % Esquema comparativo entre planificación maestro-esclavo y peer.
    \caption{Comparación entre las arquitecturas de planificación \textit{master/slave} y \textit{peer}. En la primera, un único procesador controla la planificación; en la segunda, todos los procesadores pueden ejecutar el núcleo y planificar procesos de forma coordinada.}
\end{figure}
...
\begin{figure}[!ht]
  \centering
  % Figura basada en la Figura 10.1 del libro.
...
\begin{figure}[!ht]
    \centering
    % Esquema de Load Sharing.
    \caption{Planificación mediante \textit{Load Sharing}. Todos los procesadores extraen hilos de una cola global de listos. Algunas implementaciones incorporan colas locales para mejorar la localidad de caché y reducir la contención sobre la cola compartida.}
\end{figure}
