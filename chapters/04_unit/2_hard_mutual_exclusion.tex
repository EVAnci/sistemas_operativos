...
int cerrojo; /* variable compartida */
...
Durante toda la ejecución se cumple la siguiente invariante:
\[
...
\begin{itemize}
    \item Utilizan \textit{busy waiting}, por lo que un proceso consume tiempo de CPU mientras espera acceder a la sección crítica.
    \item Puede producirse \textit{inanición}, ya que no existe garantía de que todos los procesos obtengan eventualmente acceso.
    \item Puede producirse \textit{interbloqueo} por inversión de prioridades. Por ejemplo, si un proceso de baja prioridad entra en la sección crítica y es interrumpido por otro de mayor prioridad que intenta acceder al mismo recurso, este último quedará esperando indefinidamente mientras impide que el primero continúe su ejecución.
\end{itemize}
