...
En el Intento I, la variable \texttt{turn} denota identidad: indica a qué proceso le toca entrar, lo que impone una alternancia estricta. En la solución por hardware, la variable \texttt{cerrojo} denota estado: indica si la Sección Crítica está ocupada o libre. Este cambio transforma el acceso de una cesión obligatoria del turno a un modelo de contención libre.

\paragraph{Compare and Swap (CAS)}

La instrucción \textit{compare and swap} (CAS) compara el contenido de una posición de memoria con un valor esperado. Si ambos coinciden, reemplaza el contenido por un nuevo valor; en caso contrario, la memoria permanece inalterada. Toda la operación se ejecuta de forma atómica.
...
El siguiente algoritmo implementa exclusión mutua utilizando una variable compartida \texttt{bolt}, inicialmente igual a 0. Un proceso solo puede entrar en su sección crítica si logra cambiar atómicamente el valor de \texttt{bolt} de 0 a 1. Los demás procesos permanecen en \textit{busy waiting} hasta que el recurso sea liberado.
...
En el siguiente algoritmo, cada proceso dispone de una variable local \texttt{key}, inicializada en 1, mientras que la variable compartida \texttt{bolt} comienza en 0. Mediante la operación atómica \texttt{exchange}, un proceso obtiene acceso a la sección crítica únicamente cuando consigue que \texttt{bolt} pase a valer 1. Al salir, restablece su valor a 0.
...
Si $\text{bolt}=0$, ningún proceso se encuentra en la sección crítica. Si $\text{bolt}=1$, exactamente un proceso está ejecutando su sección crítica, siendo aquel cuyo $\text{key}=0$.
