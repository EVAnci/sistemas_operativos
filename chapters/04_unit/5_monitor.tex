...
\begin{ccode}[title=Métodos y variables del monitor]
...
\begin{itemize}
  \item buffer[N]: Arreglo de tamaño N para guardar datos.
  \item nextin y nextout: Índices para recorrer el arreglo de forma circular (al llegar a \(N-1\), vuelve a 0 gracias al operador módulo \% N).
  \item count: Cantidad de elementos almacenados en ese instante.
  \item notfull y notempty: Variables de condición para pausar al productor si el buffer está lleno, o al consumidor si está vacío.
\end{itemize}

En este ejemplo, solo un proceso a la vez puede estar ejecutando código dentro del monitor (boundedbuffer). No se necesita un semáforo mutex. Si el Productor está dentro de append, el Consumidor no puede entrar a take hasta que el Productor salga (o se bloquee).
