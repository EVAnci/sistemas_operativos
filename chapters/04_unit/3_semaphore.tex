...
En términos simples \texttt{semWaitB} comprueba si el recurso está siendo utilizado, y si es así, bloquea el proceso. Por otro lado \texttt{semSignalB} comprueba si hay procesos en la cola, y si no hay, simplemente coloca el valor de \texttt{s.value} en uno.

Visto esto, el semáforo binario es un caso particular del semáforo contador, de esta manera, si el semáforo contador puede hacer todo lo que hace el binario, ¿por qué existe el semáforo binario? 
A nivel del procesador (y a nivel de hardware), ejecutar operaciones aritméticas con enteros y comprobar condiciones complejas en memoria de forma atómica (sin interrupciones) puede ser costoso.
...
En los sistemas operativos modernos, el concepto de semáforo binario evolucionó hacia el \textit{Mutex} (Mutual Exclusion lock). Aunque parecen lo mismo, el Mutex añade una regla crucial que el semáforo binario no tiene: propiedad (ownership).
\begin{itemize}
  \item En un semáforo binario, el proceso A puede hacer \texttt{semWaitB} y el proceso B puede hacer \texttt{semSignalB}.
  \item En un Mutex, únicamente el proceso que bloqueó el recurso está autorizado a liberarlo.
\end{itemize}
Esta restricción que añade el Mutex permite solucionar problemas graves de sistemas operativos.
